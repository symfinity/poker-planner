# Contributing to Poker Planner

> **Read-only mirror.** This repository (`symfinity/poker-planner`) is published from the Symfinity product monorepo (`packages/poker-planner/`). **Do not expect pull requests to be merged here.**

## Report bugs or request features

Open an [issue](https://github.com/symfinity/poker-planner/issues) on this repository.
...
Open a pull request on the **product monorepo** (`packages/poker-planner/`) on a `feature/{spec-id}-{slug}` branch. Do not rely on merging on this mirror.
